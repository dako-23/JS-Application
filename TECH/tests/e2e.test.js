    ...
    "catalog": [
        {
            ...
            "_ownerId": "0001",
            "type": "Nanotechnology",
            "image-url": "/images/Nanotechnology.png",
...
describe("E2E tests", function () {
    ...
    describe("Dashboard Page [ 15 Points ]", function () {
        ...
        it("Check Solutions have correct images [ 2.5 Points ]", async function () {
            ...
            expect(images[0]).to.contains(`${encodeURI(data[0]['image-url'])}`);
            expect(images[1]).to.contains(`${encodeURI(data[1]['image-url'])}`);
            expect(images[2]).to.contains(`${encodeURI(data[2]['image-url'])}`);
        });
    ...
    describe("CRUD [ 50 Points ]", () => {
        describe('Create [ 12.5 Points ]', function () {
            ...
            it("Create makes correct API call [ 2.5 Points ]", async function () {
                ...
                await imageElement.fill(data['image-url']);
                await descriptionElement.fill(data.description);
                await additionalInfoElement.fill(data['more-info']);
            ...
            it("Create sends correct data [ 2.5 Points ]", async function () {
                ...
                expect(postData['image-url']).to.equal(data['image-url']);
                expect(postData.description).to.equal(data.description);
                expect(postData['more-info']).to.equal(data['more-info']);
            });
        ...
        describe('Details [ 10 Points ]', function () {
            ...
            it("Details with guest calls shows correct info [ 2.5 Points ]", async function () {
                ...
                expect(imageSrc).to.contains(data['image-url']);
                expect(type).to.contains(data.type);
                expect(description).to.contains(data.description);
                expect(learnMore).to.contains(data['more-info']);
        ...
        describe('Edit [ 17.5 Points ]', function () {
            ...
            it("Edit should populate form with correct data [ 2.5 Points ]", async function () {
                ...
                expect(inputs[1]).to.contains(data['image-url']);
                expect(textareas[0]).to.contains(data.description);
                expect(textareas[1]).to.contains(data['more-info']);
            });
            ...
            it("Edit sends information to the right API [ 2.5 Points ]", async function () {
                ...
                modifiedData['image-url'] = 'Image Test';
                modifiedData.description = 'Description Test';
                modifiedData['more-info'] = 'Learn More Test';
                ...
                await imageInput.fill(modifiedData['image-url']);
                await descriptionInput.fill(modifiedData.description);
                await learnMoreInput.fill(modifiedData['more-info']);
            ...
            it("Edit sends correct information [ 2.5 Points ]", async function () {
                ...
                expect(postData['image-url']).to.contains(modifiedData['image-url']);
                expect(postData.description).to.contains(modifiedData.description);
                expect(postData['more-info']).to.contains(modifiedData['more-info']);
            });
